...
public:
    bool findBobPath(vector<vector<int>>& adj,int bob,int parent,vector<int>& curr_path,vector<int>& bob_path){
        ...
        for(int neighbour: adj[bob]){
            if(neighbour!=parent and findBobPath(adj,neighbour,bob,curr_path,bob_path))
                return true;
        }
        curr_path.pop_back();
        return false;
    ...
    int findMaxIncomeForAlice(vector<vector<int>>& adj,int alice,int parent,vector<int>& amount){
        ...
        for(int neighbour: adj[alice]){
            if(neighbour!=parent){
                int income = findMaxIncomeForAlice(adj,neighbour,alice,amount);
